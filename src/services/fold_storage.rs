...
impl MemoryFrontmatter {
    ...
    /// Convert frontmatter to a Memory.
    /// Note: project_id must be set by the caller.
    pub fn to_memory(&self) -> Memory {
        let _hash_prefix = if self.id.len() >= 2 {
            self.id[0..2].to_string()
        } else {
            String::new()
        };
...
impl FoldStorageService {
    ...
    /// then deserializes the frontmatter into a Memory.
    pub fn parse_memory_file(&self, content: &str) -> Result<(Memory, String)> {
