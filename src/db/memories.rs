...
pub async fn create_memory(pool: &DbPool, input: CreateMemory) -> Result<Memory> {
    ...
    sqlx::query_as::<_, Memory>(
        ...
        INSERT INTO memories (
            id, project_id, repository_id, type, title, content, content_hash,
...
mod tests {
    ...
    async fn test_create_and_get_memory() {
        ...
        assert_eq!(fetched.content, "This is a test memory");
    ...
    async fn test_upsert_memory() {
        ...
        assert_eq!(mem1.content, "Original content");

        // Update via upsert
        let mut updated_input = input;
        updated_input.content = "Updated content".to_string();
