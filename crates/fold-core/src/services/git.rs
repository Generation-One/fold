...
use crate::services::{
    fold_storage, EmbeddingService, FoldStorageService, MemoryService, QdrantService,
};
...
impl GitService {
    ...
    async fn scan_fold_directory(&self, repo_path: &Path) -> Result<Vec<String>> {
        fold_storage::scan_fold_directory(&self.fold_storage, repo_path).await
    }
    ...
    pub async fn sync_from_remote(&self, project: &Project) -> Result<SyncStats> {
        ...
        for memory_id in memory_ids {
            ...
            match fold_storage::read_memory(&self.fold_storage, &repo_path, &memory_id).await {
