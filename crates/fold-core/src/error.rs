...
impl From<fold_qdrant::Error> for Error {
    fn from(err: fold_qdrant::Error) -> Self {
        Self::VectorStore(err.to_string())
    }
}

impl From<fold_storage::Error> for Error {
    fn from(err: fold_storage::Error) -> Self {
        match err {
            fold_storage::Error::FileNotFound(msg) => Self::FileNotFound(msg),
            fold_storage::Error::InvalidInput(msg) => Self::InvalidInput(msg),
            fold_storage::Error::Internal(msg) => Self::Internal(msg),
            fold_storage::Error::Io(e) => Self::Internal(e.to_string()),
        }
    }
}
