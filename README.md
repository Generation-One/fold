# Fold Server - Backend Documentation

The Fold backend is a Rust-based semantic memory system built with Axum, SQLite, and Qdrant. It manages memory indexing, vector embeddings, knowledge graph operations, and API serving.
...
**Functions:**
- `summarize_code()` - Extract title, summary, keywords, tags
- `analyse_content()` - Extract context and metadata
- `suggest_links()` - Propose memory relationships
...
1. Find 5 nearest vector neighbours
2. Ask LLM: "Should we link?"
3. Create memory_links if yes
4. Update neighbour metadata
5. Write wiki-style links to fold file
